...
<table className="border-collapse w-full">
  ...
  <tbody>
    {trackingSessions.map((session) => (
      <tr key={session.id} className='relative bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0'>
        <td className='w-1/5 p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static'>{session.endTime ? formatDate(session.startTime) : '---'}</td>
        <td className='w-1/5 p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static'>{session.startTime ? formatTime(session.startTime) : '---'}</td>
        <td className='w-1/5 p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static'>{session.endTime ? formatTime(session.endTime) : '---'}</td>
        ...
        <td className='p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static'>
            <button className='text-red-500 hover:text-gray-600' onClick={() => handleDeleteSession(session.id)}>X</button>
        </td>
      </tr>
    ))}
  </tbody>
